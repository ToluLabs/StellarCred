...
Credentials may also derive a protocol-scoped nullifier from the secret and the app context:

```text
nullifier = Poseidon2([secret, app_context], 2)
```

This public output is intentionally context-bound. The same credential reused in the same application context yields the same nullifier and is rejected; the same credential in a different application context produces a distinct nullifier, preserving unlinkability across protocols.
